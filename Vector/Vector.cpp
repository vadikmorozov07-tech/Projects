...
Vector::Vector(const ValueType* rawArray, const size_t size, float coef)
    : _data(nullptr), _size(size), _capacity(size), _multiplicativeCoef(coef)
{
    if (size > 0)
    {
...
void Vector::insert(const ValueType& value, size_t pos)
{
    ...
    if (_size == _capacity)
    {
        size_t newCapacity = (_capacity == 0) ? 1*_multiplicativeCoef : static_cast<size_t>(_capacity * _multiplicativeCoef);
        reserve(newCapacity);
    }
